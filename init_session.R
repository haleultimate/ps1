source("stock_list.R")  #init functions(stock_list,clear_workspace)
stock_list()
clear_workspace()       #any functions loaded before this will be deleted
...
end_date <- "2013-03-31"
if (!exists("stx_list.old",envir=com.env)) {         #only load if stx_list has changed
  ...
  com.env$stx_list.old <- com.env$stx_list
} else if (!identical(com.env$stx_list,com.env$stx_list.old)) {
  isNameinStxold <- com.env$stx_list %in% com.env$stx_list.old
...
com.env$add_var_levels <- c(5,15,20,30)
